"""
SynthText Dataset Generator (Multiprocessing Enabled)
=====================================================
...
Example
-------
python synth_dataset_mulitiproc.py \
...
def iou_aabb(a: Tuple[int, int, int, int], b: Tuple[int, int, int, int]) -> float:
    ...
    inter = iw * ih
    area = (ax2 - ax1) * (ay2 - ay1) + (bx2 - by1) * (by2 - by1) - inter
...
def render_text_mask(text: str, params: RenderParams) -> Tuple[Image.Image, List[List[Tuple[float, float]]]]:
    ...
    est_w = int((params.font_size_px * 0.65 + params.letter_spacing_px) * max(1, len(text)) + 50) # Added padding
    est_h = int(params.font_size_px * 3.5) # Added padding
    ...
    for ch in text:
        try:
            ...
            # --- Only add polygon and advance x if successful ---
            ...
            # Catch ALL OSErrors during font rendering (e.g., "bitmap missing", "context too long", "corrupt table")
            # Print a generic warning and safely skip this character.
            print(f"[Warn] Font {Path(params.font_path).name} failed to render char '{ch}': {e}. Skipping char.")
            pass # Loop continues to the next character
    ...
    if params.shadow_px > 0:
        try:
            ...
            # If shadow rendering fails (e.g., "context too long")
            # Print a warning and safely skip adding the shadow.
            print(f"[Warn] Font {Path(params.font_path).name} failed to render shadow: {e}. Skipping shadow.")
            pass # 'canvas' remains the non-shadowed version, which is fine
    ...
    if bbox:
        ...
        # Handle case where nothing was rendered (e.g., all chars failed)
...
def apply_perspective_and_rotate(fg_rgba: Image.Image, geo: Dict[str, Any]) -> Tuple[Image.Image, np.ndarray]:
    ...
    # --- Fix for perspective clipping ---
    # Find the bounding box of the destination points
    min_x = int(np.floor(min(p[0] for p in dst)))
    min_y = int(np.floor(min(p[1] for p in dst)))
    max_x = int(np.ceil(max(p[0] for p in dst)))
    max_y = int(np.ceil(max(p[1] for p in dst)))
    
    new_w = max_x - min_x
    new_h = max_y - min_y
    
    # We need to shift the destination points to fit in a new canvas of size (new_w, new_h)
    # And we also need to translate the *source* image to the center of a larger canvas
    # so that the rotation doesn't get clipped.
    
    # Let's use a simpler approach: calculate the homography M,
    # then calculate the new bounding box, then warp.
    # We need a translation matrix to move the points so min_x, min_y become (0,0)
    T = np.float32([[1, 0, -min_x], [0, 1, -min_y], [0, 0, 1]])
    
    # M maps from src (in w2,h2) to dst (in original w2,h2 space)
    M_orig = cv2.getPerspectiveTransform(src, dst)
    
    # M_new maps from src (in w2,h2) to the *new* canvas (in new_w, new_h space)
    M_new = T @ M_orig

    out = cv2.warpPerspective(cv2.cvtColor(np.array(img_rot), cv2.COLOR_RGBA2BGRA), M_new, (new_w, new_h),
                              flags=cv2.INTER_LINEAR, borderMode=cv2.BORDER_CONSTANT, borderValue=(0, 0, 0, 0))
    
    out_img = Image.fromarray(cv2.cvtColor(out, cv2.COLOR_BGRA2RGBA))
    return out_img, M_new
...
def poisson_blend(bg_bgr: np.ndarray, fg_bgr: np.ndarray, mask: np.ndarray, x: int, y: int) -> np.ndarray:
    ...
    try:
        # Check boundaries
        if center[0] >= W or center[1] >= H or center[0] <= 0 or center[1] <= 0:
             raise ValueError("Center point is outside image boundaries")
        blended = cv2.seamlessClone(fg_bgr, bg_bgr, mask, center, cv2.MIXED_CLONE)
        return blended
    except Exception:
        # Fallback to alpha blend in case of any error (e.g., boundary)
        return alpha_blend(bg_bgr[y:y+h, x:x+w], fg_bgr, mask[y:y+h, x:x+w])
...
# VVVV NEW Helper Functions (must be at global scope) VVVV

_GLOBAL_GENERATOR_INSTANCE = None  # <-- 修改点: 全局变量用于存储生成器实例

def _init_worker(cfg: 'GenConfig'):
    """
    初始化工作进程。
    创建 SynthTextDatasetGenerator 实例并将其存储在全局变量中。
    这个函数在每个工作进程启动时【只运行一次】。
    """
    global _GLOBAL_GENERATOR_INSTANCE
    
    # 1. 在此处创建实例。
    #    __init__ (包括磁盘扫描) 将在此处运行一次。
    _GLOBAL_GENERATOR_INSTANCE = SynthTextDatasetGenerator(cfg)
    
    # 2. 为此工作进程设置一个基础随机种子
    #    (我们仍将在 _generate_task 中设置特定于任务的种子)
    set_seed(cfg.seed + os.getpid())


def _generate_task(idx: int) -> Optional[Dict[str, Any]]: # <-- 修改点: 返回值
    """
    由单个工作进程执行的实际任务（生成一个图像）。
    现在：它从全局变量获取生成器实例，而不是重新创建它。
    """
    global _GLOBAL_GENERATOR_INSTANCE # <-- 修改点: 获取全局实例
    if _GLOBAL_GENERATOR_INSTANCE is None:
        print("错误: 工作进程未正确初始化。")
        return None
    
    # 1. 从全局变量获取预先初始化的生成器实例
    gen = _GLOBAL_GENERATOR_INSTANCE
    cfg = gen.cfg # 从实例中获取配置
    
    # 2. 为此【特定任务】设置唯一的随机种子
    #    这确保了无论哪个worker执行哪个idx，结果都是可复现的
    set_seed(cfg.seed + idx)
    
    # 3. 执行生成
    res = gen.generate_one(idx, level=2)
    ...
    # --- VVVV (这部分逻辑保持不变，它已经很好) VVVV ---
    try:
        # 4. 从注解中获取图像ID
        image_id = ann['image_id']
        
        # 5. 构建输出路径
        out_images = Path(cfg.out_dir, 'images')
        ...
        print(f"[Error in Worker {idx}] 无法保存图像或处理: {e}")
        return None
...
class SynthTextDatasetGenerator:
    def __init__(self, cfg: GenConfig):
        self.cfg = cfg
        # set_seed(cfg.seed) # Seed is now set *per-worker* in _generate_task
        
        # These properties will be initialized in both main and child processes
        ...
        # Only print warnings in the main process
        if not self.bg_files and (multiprocessing.parent_process() is None):
            print('[warn] No backgrounds found; using gradient backgrounds for pipeline sanity check.')
        if not self.font_files and (multiprocessing.parent_process() is None):
    ...
    def _build_instance(self, bg_img: Image.Image, text: str, exist_bboxes: List[Tuple[int, int, int, int]]) -> Optional[Dict[str, Any]]:
        ...
        if text_rgba.size[0] <= 1 or text_rgba.size[1] <= 1:
            # Render failed (e.g., all chars were bad)
            return None
        ...
        if w2 <= 1 or h2 <= 1:
            return None # Geometry transform failed or created empty image
        ...
        # Calculate slices for fg and alpha
        fg_x1, fg_y1 = x1 - x, y1 - y
        fg_x2, fg_y2 = x2 - x, y2 - y

        if fg_x1 >= fg_x2 or fg_y1 >= fg_y2:
            return None # Instance is completely outside the image

        roi_bg = bg_bgr[y1:y2, x1:x2]
        roi_fg = fg_bgr[fg_y1:fg_y2, fg_x1:fg_x2]
        roi_a = fg_a[fg_y1:fg_y2, fg_x1:fg_x2]
        ...
        try:
            if mode == 'alpha':
                merged[y1:y2, x1:x2] = alpha_blend(roi_bg, roi_fg, roi_a)
            elif mode == 'screen':
                merged[y1:y2, x1:x2] = screen_blend(roi_bg, roi_fg, roi_a)
            else:
                # Poisson blending needs a full-size fg and mask
                mask = np.zeros((Hc, Wc), dtype=np.uint8)
                mask[y1:y2, x1:x2] = roi_a
                fg_full = np.zeros_like(bg_bgr)
                fg_full[y1:y2, x1:x2] = roi_fg
                
                # Check center point for seamlessClone
                center_x = (x1 + x2) // 2
                center_y = (y1 + y2) // 2
                if center_x >= Wc or center_y >= Hc or center_x <= 0 or center_y <= 0:
                    raise ValueError("Center for Poisson blend is out of bounds.")
                
                merged = cv2.seamlessClone(fg_full, bg_bgr, mask, (center_x, center_y), cv2.MIXED_CLONE)
        except Exception:
             # Fallback to alpha blend if compositing fails
             merged[y1:y2, x1:x2] = alpha_blend(roi_bg, roi_fg, roi_a)
        ...
        if random.random() < 0.25:
            ...
            # Ensure occlusion coordinates are valid
            ox1, oy1 = max(0, ox), max(0, oy)
            ox2, oy2 = min(Wc, ox + occ_w), min(Hc, oy + occ_h)
            if ox1 < ox2 and oy1 < oy2:
                 merged[oy1:oy2, ox1:ox2] = bg_bgr[oy1:oy2, ox1:ox2]
    ...
    def generate_one(self, idx: int, level: int = 2) -> Optional[Tuple[np.ndarray, Dict[str, Any]]]:
        ...
        # Note: Seeding is handled by the worker function `_generate_task`
        ...
        # print(f"[Debug] Image {idx}: Failed to generate valid image after {self.cfg.per_image_retry} retries.")
        return None

    # VVVV This is the MODIFIED run() method for Multiprocessing VVVV
    def run(self, level: int = 2):
        ...
        start_idx = 0
        print(f"Checking for existing images in {out_images} (prefix: '{self.cfg.split}_')...")
        ...
        if max_idx >= 0:
            start_idx = max_idx + 1
            print(f"Found existing images. Resuming generation from index {start_idx}.")

        if start_idx >= self.cfg.num_images:
             print(f"Generation task already complete (Found {start_idx}/{self.cfg.num_images} images). Exiting.")
             return
        # --- Resume Logic (Ends) ---
        ...
        if self.cfg.save_jsonl:
            ...
            if file_mode == 'a':
                print(f"Appending annotations to existing file: {jsonl_path}")
        
        # --- Multiprocessing Logic (Start) ---
        
        num_workers = max(1, multiprocessing.cpu_count() - 2)
        print(f"Starting generation using {num_workers} worker processes...")

        tasks = range(start_idx, self.cfg.num_images)
        ...
        with multiprocessing.Pool(processes=num_workers, 
            ...
            pbar = tqdm(pool.imap_unordered(_generate_task, tasks), 
                        total=num_tasks_to_run, 
                        desc=f'generating (split: {self.cfg.split})')

            # --- VVVV 修改点: 主循环现在非常轻量 VVVV ---
            
            # 5. Collect results (which are *only* the 'ann' dicts)
            for ann in pbar:
                if ann is None:
                    # This means the worker failed to generate or save
                    continue
                
                # The main process ONLY writes the annotation
                if jsonl_fp:
                    jsonl_fp.write(json.dumps(ann, ensure_ascii=False) + '\n')
                else:
                    # This is the fallback for per-image JSONs
                    image_id = ann['image_id']
                    with open(out_labels / f'{image_id}.json', 'w', encoding='utf-8') as f:
                        json.dump(ann, f, ensure_ascii=False, indent=2)
            # --- ^^^^ 修改点: 主循环结束 ^^^^ ---

        # --- Multiprocessing Logic (End) ---

        if jsonl_fp:
            jsonl_fp.close()
        
        print(f"Generation complete. Output in: {self.cfg.out_dir}")
    # ^^^^ run() method modification ends ^^^^
...
def main():
    ...
    cfg = GenConfig(
        ...
        split=args.split # Use the split from args
    )
    # The main process creates one instance just to call .run()
